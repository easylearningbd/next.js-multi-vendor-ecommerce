"use client";

import { useState, useTransition } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import toast from "react-hot-toast";
import { toggleBrandStatus } from "@/app/(admin)/admin/brands/actions";
import type { BrandListItem } from "@/lib/brand-types";
import { Icon } from "@/components/dashboard/Icon";
import { BrandStatusBadge } from "./BrandStatusBadge";
import { BrandsToolbar } from "./BrandsToolbar";
import { BrandsPagination } from "./BrandsPagination";
import { BrandFormModal } from "./BrandFormModal";
import { DeleteBrandDialog } from "./DeleteBrandDialog";

const GRID = "grid-cols-[44px_64px_minmax(160px,1.4fr)_minmax(140px,1fr)_90px_150px_128px]";

export function BrandsManager({
  brands,
  total,
  page,
  pageSize,
  totalPages,
  hasFilters,
  errored,
}: {
  brands: BrandListItem[];
  total: number;
  page: number;
  pageSize: number;
  totalPages: number;
  hasFilters: boolean;
  errored: boolean;
}) {
  const router = useRouter();
  const [formModal, setFormModal] = useState<{ brand?: BrandListItem } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<BrandListItem | null>(null);
  const [pendingToggle, setPendingToggle] = useState<string | null>(null);
  const [, startTransition] = useTransition();

  function handleToggle(b: BrandListItem) {
    setPendingToggle(b.id);
    startTransition(async () => {
      const res = await toggleBrandStatus(b.id);
      if (res.success) {
        toast.success(`${b.name} is now ${res.data?.status === "ACTIVE" ? "active" : "inactive"}`);
        router.refresh();
      } else {
        toast.error(res.error ?? "Couldn't update status.");
      }
      setPendingToggle(null);
    });
  }

  const actionBtn =
    "flex h-8 w-8 items-center justify-center rounded-md border transition-colors";

  return (
    <>
      {/* Page header */}
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className="flex h-9 w-9 items-center justify-center rounded-md bg-iris-50 text-iris-500">
            <Icon name="tag" size={20} strokeWidth={1.9} />
          </span>
          <h1 className="m-0 font-display text-[24px] font-extrabold tracking-[-0.01em] text-ink">Brands</h1>
          <span className="flex h-[26px] min-w-[30px] items-center justify-center rounded-full bg-line-soft px-2.5 font-display text-[13px] font-bold text-ink-soft">
            {total}
          </span>
        </div>
        <button
          type="button"
          onClick={() => setFormModal({})}
          className="flex h-[46px] items-center gap-2 rounded-md bg-iris-500 px-5 font-display text-[13px] font-bold text-white transition-colors hover:bg-iris-600"
        >
          <Icon name="plus" size={17} strokeWidth={2.2} />
          Add Brand
        </button>
      </div>

      {/* Card */}
      <div className="rounded-2xl border border-line-soft bg-surface p-[22px_24px] shadow-xs">
        <BrandsToolbar />

        {errored ? (
          <StateBlock
            tone="error"
            icon="alert"
            title="Couldn't load brands"
            text="Something went wrong while loading the brand list. Please try again."
            action={
              <button
                type="button"
                onClick={() => router.refresh()}
                className="flex h-[46px] items-center gap-2 rounded-md bg-iris-500 px-6 font-sans text-[13.5px] font-semibold text-white transition-colors hover:bg-iris-600"
              >
                <Icon name="refresh" size={16} strokeWidth={2} />
                Try again
              </button>
            }
          />
        ) : brands.length === 0 ? (
          hasFilters ? (
            <StateBlock
              tone="empty"
              icon="search"
              title="No brands match your filters"
              text="Try a different name or clear the status filter."
            />
          ) : (
            <StateBlock
              tone="empty"
              icon="tag"
              title="No brands yet"
              text="Create your first brand to start organizing the catalog."
              action={
                <button
                  type="button"
                  onClick={() => setFormModal({})}
                  className="flex h-[46px] items-center gap-2 rounded-md bg-iris-500 px-6 font-sans text-[13.5px] font-semibold text-white transition-colors hover:bg-iris-600"
                >
                  <Icon name="plus" size={16} strokeWidth={2.2} />
                  Add Brand
                </button>
              }
            />
          )
        ) : (
          <>
            <div className="overflow-x-auto">
              <div className="min-w-[840px] overflow-hidden rounded-lg border border-line-soft">
                <div
                  className={`grid ${GRID} gap-3.5 bg-field p-[14px_18px] font-sans text-[11px] font-semibold uppercase tracking-[0.04em] text-muted`}
                >
                  <span>#</span>
                  <span>Image</span>
                  <span>Brand name</span>
                  <span>Slug</span>
                  <span>Products</span>
                  <span>Status</span>
                  <span className="text-right">Actions</span>
                </div>

                {brands.map((b, i) => (
                  <div
                    key={b.id}
                    className={`grid ${GRID} items-center gap-3.5 border-t border-line-soft p-[12px_18px] transition-colors hover:bg-bg-subtle`}
                  >
                    <span className="font-sans text-[13px] text-muted">{(page - 1) * pageSize + i + 1}</span>

                    <div className="h-[46px] w-[46px] overflow-hidden rounded-md border border-line-soft bg-field">
                      {b.image ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={b.image} alt={b.name} className="h-full w-full object-cover" />
                      ) : (
                        <span className="flex h-full w-full items-center justify-center text-muted-soft">
                          <Icon name="image" size={18} strokeWidth={1.8} />
                        </span>
                      )}
                    </div>

                    <div className="min-w-0">
                      <div className="truncate font-sans text-[13.5px] font-semibold text-ink">{b.name}</div>
                    </div>

                    <span className="truncate font-mono text-[12px] text-muted">{b.slug}</span>

                    <span className="font-sans text-[13px] text-ink-soft">{b.productCount}</span>

                    <div className="flex items-center gap-2.5">
                      <button
                        type="button"
                        role="switch"
                        aria-checked={b.status === "ACTIVE"}
                        aria-label={`Toggle ${b.name} status`}
                        disabled={pendingToggle === b.id}
                        onClick={() => handleToggle(b)}
                        className={`relative h-[22px] w-[38px] flex-none rounded-full transition-colors disabled:opacity-60 ${
                          b.status === "ACTIVE" ? "bg-iris-500" : "bg-[#e4e2e9]"
                        }`}
                      >
                        <span
                          className={`absolute top-[3px] h-4 w-4 rounded-full bg-white transition-all ${
                            b.status === "ACTIVE" ? "left-[19px]" : "left-[3px]"
                          }`}
                        />
                      </button>
                      <BrandStatusBadge status={b.status} />
                    </div>

                    <div className="flex justify-end gap-2">
                      <Link
                        href={`/admin/brands/${b.id}`}
                        aria-label={`View ${b.name}`}
                        className={`${actionBtn} border-success-bg bg-[#f1fbf6] text-success hover:brightness-95`}
                      >
                        <Icon name="eye" size={15} strokeWidth={2} />
                      </Link>
                      <button
                        type="button"
                        aria-label={`Edit ${b.name}`}
                        onClick={() => setFormModal({ brand: b })}
                        className={`${actionBtn} border-iris-100 bg-iris-50 text-iris-500 hover:bg-iris-100`}
                      >
                        <Icon name="edit" size={15} strokeWidth={2} />
                      </button>
                      <button
                        type="button"
                        aria-label={`Delete ${b.name}`}
                        onClick={() => setDeleteTarget(b)}
                        className={`${actionBtn} border-[#f6d9da] bg-error-bg text-error hover:brightness-95`}
                      >
                        <Icon name="trash" size={15} strokeWidth={2} />
                      </button>
                    </div>
                  </div>
                ))}
              </div>
            </div>

            <BrandsPagination page={page} totalPages={totalPages} total={total} pageSize={pageSize} />
          </>
        )}
      </div>

      {formModal && (
        <BrandFormModal open onClose={() => setFormModal(null)} brand={formModal.brand} />
      )}
      {deleteTarget && (
        <DeleteBrandDialog open onClose={() => setDeleteTarget(null)} brand={deleteTarget} />
      )}
    </>
  );
}

function StateBlock({
  tone,
  icon,
  title,
  text,
  action,
}: {
  tone: "empty" | "error";
  icon: React.ComponentProps<typeof Icon>["name"];
  title: string;
  text: string;
  action?: React.ReactNode;
}) {
  return (
    <div className="flex flex-col items-center px-8 py-16 text-center">
      <span
        className={`mb-[22px] flex h-[78px] w-[78px] items-center justify-center rounded-2xl ${
          tone === "error" ? "bg-error-bg text-error" : "bg-iris-50 text-iris-400"
        }`}
      >
        <Icon name={icon} size={34} strokeWidth={1.7} />
      </span>
      <div className="font-display text-[20px] font-bold leading-[1.2] text-ink">{title}</div>
      <p className="mx-auto mb-6 mt-3 max-w-[340px] font-sans text-[14px] leading-[1.5] text-muted">{text}</p>
      {action}
    </div>
  );
}
