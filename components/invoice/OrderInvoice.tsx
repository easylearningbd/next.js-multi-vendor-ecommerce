...
import type { ConfirmationOrder } from "@/lib/shop/order";
...
/**
 * The printable invoice document for one full order (all sellers, grand total).
 * Shared by the customer invoice route and the admin (full-order) invoice route —
 * one generator, two callers. Purely presentational; the caller owns auth, the
 * data fetch, and the print/download controls.
 */
export function OrderInvoice({ order }: { order: ConfirmationOrder }) {
    ...
    <div className="mx-auto max-w-[820px] bg-surface px-10 py-9 text-ink shadow-[0_1px_3px_rgba(20,18,31,0.08)] print:max-w-none print:px-0 print:py-0 print:shadow-none">
      ...
      <div className="flex items-start justify-between border-b border-line pb-6">
        <div>
          ...
          <div className="mt-1 font-sans text-[12px] text-muted">Multi-vendor marketplace</div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 border-b border-line py-6 sm:grid-cols-3">
        ...
        <div>
          ...
          <div className="font-sans text-[13px] leading-[1.6] text-ink-soft">
            ...
            <div className="text-muted">{order.paymentStatus === "PAID" ? "Paid" : "Unpaid"}</div>
      ...
      <div className="mt-8 border-t border-line pt-5 text-center font-sans text-[12px] text-muted">
        Thank you for shopping with Covet. This invoice was generated for order {order.orderNumber}.
      </div>
    </div>
  );
}
