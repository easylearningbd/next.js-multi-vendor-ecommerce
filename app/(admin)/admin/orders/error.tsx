...
export default function AdminOrdersError({ reset }: { error: Error; reset: () => void }) {
  return (
    <div className="flex flex-col items-center rounded-[18px] border border-line-soft bg-surface px-8 py-16 text-center">
      ...
      <div className="font-display text-[20px] font-bold text-ink">Couldn&apos;t load orders</div>
      <p className="mx-auto mt-3 max-w-[340px] font-sans text-sm leading-[1.5] text-muted">
        Something went wrong while loading the marketplace orders. Please try again.
      </p>
      <button
        type="button"
        onClick={reset}
